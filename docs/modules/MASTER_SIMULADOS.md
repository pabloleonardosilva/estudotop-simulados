...
**Nota de status (2026-09-06):** as seções 3.18 (Anulação de questão), 3.19 (Alteração de gabarito), 3.20 (Anulação/gabarito durante tentativa em andamento) e 3.21 (Reprocessamento) descrevem a arquitetura que **agora está implementada**, com uma diferença de nomenclatura: o texto original falava em "novo status" de resultado por reprocessamento; a implementação real atualiza o próprio `simulado_results` da tentativa (preservando a tentativa original intacta) em vez de criar uma linha nova, e usa os campos já previstos no schema desde a criação da tabela (`had_live_rule_change`, `last_reprocessed_at`, `reprocess_reason` em `simulado_results`; `annulled_at`, `annulled_by`, `annulment_reason` em `simulado_questions`) — nenhuma migration nova foi necessária, eles já existiam sem uso. Fonte de verdade do código: `lib/simuladoScoring.ts` (correção pura) e `lib/server/simuladoQuestionReprocessing.ts` (orquestração). Ver `docs/Sprint-resultados.md` para a matriz final de propagação/recálculo e a garantia contra dupla bonificação.

**Nota de fechamento (2026-09-06):** atomicidade resolvida como serialização real (compare-and-swap no `UPDATE` de status, garantia nativa do Postgres/MVCC) + idempotência recuperável, deliberadamente **sem** duplicar a fórmula de correção em SQL — decisão registrada e explicada em `docs/Sprint-resultados.md`. Teste real de concorrência (execução via `Promise.all`, não só leitura de código) em `tests/simulado-question-annulment/concurrency.spec.ts`. Alerta no Banco de Questões (contagem de vínculos anulados, sem N+1) implementado em `/questoes`. Ranking do Professor testado por execução real em `tests/simulado-question-annulment/ranking.spec.ts` (função extraída para `lib/eventRanking.ts`).

**Nota de fechamento cirúrgico (2026-09-07):** dois pontos corrigidos, documentados em `docs/Sprint-resultados.md` — (1) a chave de idempotência da notificação (`student_notifications`) ganhou `revision_id`, identidade persistida em `simulado_questions.status_revision_id`/`questions.answer_key_revision_id` gerada só no momento real da transição/mudança, para que um retry da mesma revisão não duplique mas uma revisão futura distinta (mesmo tipo, ex. anular duas questões diferentes na mesma tentativa) gere um aviso novo; (2) `PUT /api/admin/simulados/[id]/questions` deixou de escrever `simulado_questions.status` diretamente — agora delega para o mesmo `setSimuladoQuestionAnnulment()` das rotas dedicadas, fechando o único bypass real encontrado numa varredura de todo o repositório. Migration nova (`20260907140000_notification_revision_identity.sql`, criada e não aplicada) e dois testes novos.

**Nota de recuperação de falha parcial (2026-09-07, mesma rodada):** `simulado_questions.pending_reconciliation_at` (nova coluna, migration `20260907130000_simulado_question_annulment_reconciliation.sql`, reaproveitando o `status_revision_id` já existente) fecha um bloqueador real — uma queda de processo no meio de `reprocessSimulado()` deixava resultados parcialmente reconciliados sem nenhum marcador objetivo de pendência, e uma nova tentativa de anular a mesma questão era rejeitada como no-op sem nunca retomar o que faltava. `processPendingReconciliationForSimulado()`/`getPendingReconciliationSummary()` (novas) e `GET`/`POST /api/admin/simulados/[id]/reconciliation` (novo, Admin-only) resolvem isso; `setSimuladoQuestionAnnulment()` tenta concluir automaticamente qualquer pendência antes de aceitar uma nova transição contraditória. Nenhuma segunda implementação de scoring foi criada — `lib/simuladoScoring.ts` continua a única fonte. Testado por execução real (100 tentativas, falha determinística na 37ª, retry completo, estado final idêntico ao baseline) em `tests/simulado-question-annulment/reconciliation-recovery.spec.ts`, que também expôs e corrigiu um segundo bug real (TopCoins podiam ficar permanentemente desatualizados para alunos já corrigidos antes de uma falha — corrigido movendo `resyncTopCoinEarnings()` para dentro do loop por tentativa). Detalhes completos em `docs/Sprint-resultados.md`.

**Incidente real de produção (2026-09-07, mesma rodada):** a anulação real da questão `ET3582` (3º Simulado de Processo Civil) expôs um terceiro bug — `reprocessSimulado()` buscava `simulado_answers` de todas as tentativas do Simulado sem paginação; com 135 tentativas × 12 questões (~1620 linhas), o limite padrão de resposta do PostgREST/Supabase (1000) cortou a consulta silenciosamente, e o motor tratou respostas reais ausentes como questão em branco — 113 de 135 resultados oficiais ficaram com nota reduzida incorretamente. Corrigido com paginação explícita e determinística (`.range()` + `.order("id")` + conferência do `count` exato, não um `.limit()` maior). `reconcileCurrentRevision()` (nova) permite reprocessar a mesma revisão já concluída para corrigir os dados, sem gerar revisão nova nem mudar status — exposta pelo endpoint já existente de reconciliação, sem rota nova. Testado na mesma escala do incidente (1620 respostas) em `tests/simulado-question-annulment/large-answer-set.spec.ts`. Dados de produção ainda não corrigidos — etapa controlada separada. Detalhes em `docs/Sprint-resultados.md`.

**Nota de análise pedagógica — guia "Insights" do Professor (2026-09-09):** nova camada de análise (não uma mudança de arquitetura de scoring/pontuação) sobre o mesmo dado já consolidado: `lib/eventInsights.ts` deriva, por Simulado/Evento, uma dificuldade por questão (`D_q = wrong/(correct+wrong)`, anulada sempre excluída, branco fora do denominador), consolidada por tópico com peso igual por questão (`D_t`) e suavizada estatisticamente em direção à média global do Simulado (`D_adjusted = (n·D_t + k·D_global)/(n+k)`, k=2) para não dar falsa confiança a tópicos com poucas questões. Reaproveita `lib/topicDifficulty.ts` (normalização de tópico, já usada pelo resultado do aluno e pelo card por participante do Ranking) e o `questionStats` já calculado por `GET /api/professor/events/[id]` — nenhuma consulta nova, nenhuma migration, nenhuma mudança em `lib/simuladoScoring.ts`, `lib/eventRanking.ts` ou nos resultados oficiais. Resumo textual final é determinístico (sem IA/API externa). Detalhes completos: `docs/Sprint-evento-de-simulado.md`, seção 105.

**Nota de refinamento de UX — guia "Insights" (2026-09-10):** ajuste de apresentação sobre a nota acima — a matemática (D_q/D_t/D_global/D_adjusted, k=2) não mudou. A lista de tópicos passou a mostrar só a dificuldade ajustada (a bruta e a confiança da amostra continuam calculadas internamente, só deixaram de aparecer na tela), com classificação renomeada para Extrema/Alta/Média/Baixa (limiares 75/50/25%, centralizados na mesma função `classifyTopicDifficultyBand`). A lista de questões mais difíceis deixou de mostrar taxa de branco e passou a abrir a questão num modal de consulta somente-leitura reaproveitando `QuestionDisplayCard` (mesmo componente já usado na aba Questões/revisão) e dados já carregados — zero fetch novo. O bloco "Mapa de domínio" foi removido por decisão de clareza do usuário. Detalhes completos: `docs/Sprint-evento-de-simulado.md`, seção 106.

**Nota de correção da coleta de dados — guia "Insights" (2026-09-10, mesma rodada):** auditoria comprovou que `GET /api/professor/events/[id]` truncava silenciosamente `simulado_answers`/`simulado_results` (mesma classe de bug do incidente `ET3582` acima — 1587 respostas reais, 1000 retornadas, no mesmo Evento) por falta de paginação, e confiava cegamente em `representative_attempt_id` sem revalidar status (12 casos reais de tentativa `in_progress`/`disqualified` incorretamente registrada como representativa). Corrigido com `fetchAllPages()` (extraída para `lib/server/supabasePagination.ts`, reaproveitada também por `simuladoQuestionReprocessing.ts`) e uma base estritamente `completed` (`completedRepresentativeAttemptIds`/`completedQuestionStats`), separada da base operacional ao vivo que a aba Questões/revisão continua usando sem alteração. Polling do painel do Professor ganhou proteção contra resposta HTTP fora de ordem (`AbortController`). Matemática dos Insights, scoring, ranking e PDF inalterados; nenhum dado histórico corrigido nesta etapa. Detalhes completos: `docs/Sprint-evento-de-simulado.md`, seção 107.
