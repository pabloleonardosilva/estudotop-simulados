...
- **Auditoria de preservação (código lido, nada alterado):** `representative_attempt_id` tem duas travas pré-existentes contra sobrescrita (`.is("representative_attempt_id", null)` em `.../attempts`; `if (!participant?.representative_attempt_id)` em `.../submit`). "Ver meus resultados" sempre usa `?attemptId=representative_attempt_id`. Dashboard do Professor/Modo Aula já documentados como baseados exclusivamente na tentativa representativa. TopCoins (`resyncTopCoinEarnings`) já é, para todo o sistema, uma recompensa por tentativa que conta com multiplicador decrescente — não duplica, recalcula do zero a cada chamada; esse comportamento é anterior a esta Sprint e vale igualmente para Jornada/avulso.
...
- **`representative_attempt_id`/`counts_toward_limit`:** nenhuma política nova — segue exatamente a mesma regra já aplicada a qualquer desclassificação (tentativa já é representativa desde a criação; `counts_toward_limit=true` consome a tentativa).
...
- [x] Dois bloqueadores restantes da auditoria final Hotmart corrigidos: token já usado reconcilia o envio local sem novo payload/Resend, e estados financeiros repetidos são no-op funcional mesmo com novo `event.id`. A migration complementar não foi alterada, continua **NÃO EXECUTADA** e aguarda auditoria finalíssima.
- [x] Último bloqueador de primeiro acesso Hotmart corrigido: lookup da entrega falha fechado, distinguindo erro de ausência real; reconciliação de token usado não depende da chave Resend. A migration complementar continua **NÃO EXECUTADA** e aguarda auditoria final.
- [x] Estado vigente em 2026-08-30: `20260830120000_complete_hotmart_admin_workflows.sql` **APLICADA E VALIDADA**; homologação interna 35/35 PASS, fixtures removidas e baseline preservado. As linhas anteriores registram o estado histórico de cada auditoria antes da aplicação.
- [ ] Homologação externa Hotmart **PARCIAL — READINESS**: HOTTOK e credenciais OAuth ausentes; endpoint público atual retornou 404 no teste negativo sem HOTTOK; não houve deploy autorizado. Antes de testes autenticados também precisam ser corrigidos e revalidados o parsing de timestamps Webhook 2.0.0 em milissegundos, o uso da credencial OAuth Basic e a seleção explícita de sandbox para refund. Nenhum webhook autenticado, OAuth, refund ou e-mail real foi executado.
- [x] Três bloqueadores locais do readiness Hotmart corrigidos: timestamps Webhook 2.0 em milissegundos, OAuth com `HOTMART_BASIC_TOKEN` e retry único após 401, e seleção fail-closed `HOTMART_ENVIRONMENT=sandbox|production`. A homologação externa continua pendente de variáveis, produto sandbox e deploy autorizado; nenhum serviço externo foi acionado.
...
## 14/09/2026 — Homologação Hotmart: OAuth, catálogos Sandbox/Produção, lookup por ambiente e card premium de Transações

- [x] OAuth Sandbox corrigido: causa raiz era `HOTMART_BASIC_TOKEN` com prefixo `"Basic "` duplicado na credencial armazenada na Vercel; após correção operacional da credencial (token puro) e redeploy, o 401 desapareceu. Log sanitizado `[HOTMART_OAUTH_DIAGNOSTIC]` (sem secrets) adicionado em `refund.ts` para diagnóstico futuro.
- [x] Catálogo Sandbox (listagem administrativa temporária + lookup por UCODE) homologado; confirmado que o catálogo Sandbox é sintético/fictício, distinto do catálogo real da conta.
- [x] UCODE Hotmart real pode ficar fora do UUID RFC 4122 estrito; validação trocada para estrutura hexadecimal 8-4-4-4-12 (`HOTMART_UCODE_PATTERN`), case-insensitive, com normalização para minúsculas antes de qualquer comparação/gravação (lookup, criação de mapping, busca de mapping no webhook).
- [x] Credenciais read-only de Production (`HOTMART_PRODUCTION_CLIENT_ID/CLIENT_SECRET/BASIC_TOKEN`) isoladas em módulo próprio, sem depender de `HOTMART_ENVIRONMENT` e sem uso por webhook/refund/processor. Catálogo real de Production (listagem + lookup) homologado, listando corretamente produtos reais da conta.
- [x] Lookup por UCODE dividido explicitamente por origem (Sandbox/Produção) no formulário de vínculo, com seletor visível; "Usar este produto" nas duas listagens preenche UCODE/nome/origem sem nova consulta.
- [x] Card de Transações redesenhado em Dark Premium oficial (tokens `et-admin-dark-*` existentes, sem paleta paralela), compartilhado entre Transações/Pendências/Compras em duplicidade; mostra produto, UCODE copiável, data comercial, valor na moeda real quando ambos existem (currency nunca é presumida como "BRL" — sem os dois, mostra valor neutro com "moeda não informada" ou nada, nunca inventa dado), comprador, situação traduzida.
- [x] Botão **Vincular** corrigido nesta revisão: depende só de o produto (por UCODE) não ter mapping ativo/válido — não fica mais escondido por causa do `processing_status` da transação. Mapping ativo com destino resolvido → "Vinculado"; mapping ativo sem destino resolvido → "Destino indisponível"; mapping existente porém inativo → "Vínculo inativo" (não tratado como vínculo operacional); nenhum mapping → "Vincular", reaproveitando o modal já homologado de Pendências. "Vincular e reprocessar" só é oferecido quando o `processing_status` é elegível para reprocessamento (mesma lista do processor); caso contrário só "Salvar vínculo".
