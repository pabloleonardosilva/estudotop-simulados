...
## 10/09/2026 - Padronizacao global de dropdowns (nenhum select nativo visivel)

Regra permanente: **controles `<select>` nativos nao sao o padrao visual final do EstudoTOP Simulados.** Interfaces Dark Premium e Clean Premium devem usar os componentes de dropdown premium compartilhados (`PremiumSimpleSelect` para listas curtas sem busca, `SearchableSelect` para listas longas ou quando busca ja e util com poucas opcoes), preservando comportamento, acessibilidade (teclado completo, `aria-haspopup`/`aria-expanded`, `role="listbox"`/`role="option"`) e tema. Cada variante segue a mesma linguagem visual ja estabelecida por `SearchableSelect`: trigger + painel customizado, nunca menu nativo do navegador/SO.

Migrados nesta entrega (Dark e Clean, conforme a pagina): `/admin/logs`, `/admin/jornadas/[id]`, `/admin/raio-x-provas/[id]` e `/nova`, `/admin/alunos`, `/admin/alunos/[id]`, `/admin/alunos/novo`, `/professor/eventos/[id]`, `/simulados`, `/simulados/novo`, `/simulados/[id]/editar`, `/admin/eventos` e `[id]`, `/assuntos` e `/importar`, `/admin/professores`, `/questoes/gerar-ia`, `QuestionTemplatePicker`. `SearchableSelect` e `PremiumSimpleSelect` ganharam suporte a `disabled`; `PremiumSimpleSelect` ganhou variante `compact` para grades densas de edicao em lote (Raio-X). Detalhamento completo (props, inventario, arquivos, exceoes documentadas) em `docs/INDICE_FUNCOES_SISTEMA.md`, secoes 19.4.1 e 19.4.2.

Regra complementar sobre inputs compostos (campo de busca com icone): o `<input>` deve ser a propria caixa visual (altura/borda/radius/padding), nunca viver dentro de um wrapper com cor de fundo propria diferente da que o CSS global de `.et-admin-dark-content` forca no proprio `<input>` — um wrapper com fundo divergente cria um retangulo visivelmente diferente atras do texto. Corrigido em `/admin/configuracoes/tentativas-cadastro` (causa raiz do defeito relatado) e `/admin/logs` (mesma causa, mesmo componente duplicado).

Nao migrado nesta entrega, com motivo individual documentado: `app/questoes/page-client.tsx` (`<select>` "Status" de edicao em lote) possui diff local pre-existente de outra frente de trabalho — alteracao evitada para nao misturar diffs de sessoes diferentes, nao por risco tecnico da migracao em si. `SimpleSelectDropdown` (ja um dropdown customizado, nunca nativo, duplicado em 4 telas) e `ActivityPremiumSelect` (idem, local a `/admin/alunos/[id]`) nao foram consolidados nesta entrega — ja nao violam a regra acima (nao sao `<select>` nativo) e a consolidacao e uma melhoria de manutenibilidade, nao de conformidade visual; prioridade de estabilidade.

Nenhuma migration foi criada ou alterada. Nenhuma regra de negocio, valor, opcao ou ordenacao de filtro foi alterada. Teste estrutural novo `tests/dropdown-standardization.spec.ts` audita a ausencia de `<select>` nativo fora da excecao documentada a cada execucao. Sem commit, push ou deploy.

## 25/09/2026 - Padrao global de Campo de Pesquisa (Dark Premium e Clean Premium)

Esta entrega formaliza como regra permanente do design system a "Regra complementar sobre inputs compostos" registrada em 10/09/2026 (linha acima) e estende a auditoria a todo o sistema, apos ela ter causado um defeito visual concreto (retangulo escuro duplicado dentro do dropdown de `SearchableSelect`) identificado durante homologacao manual.

### Causa raiz (por que o retangulo duplicado aparece)

`app/globals.css` forca, com `!important`, o fundo de qualquer `<input>`/`<select>`/`<textarea>` "nu" (sem outras classes que o protejam) dentro de `.et-admin-dark-content` para `#050b13` (e dentro de `.et-admin-dark-content .et-admin-clean-content`, o equivalente claro `#ffffff`). Um componente de busca que coloca o `<input>` (com `bg-transparent`) dentro de um `<div>` com uma cor de fundo propria e diferente (ex.: `bg-white/[0.06]` claro, ou `bg-black/30`/`bg-white/[0.04]` translucido) sempre produz dois tons visiveis: o do `<div>` (Tailwind normal, nao afetado pela regra global, que so mira tags de input) e o do `<input>` (forcado pela regra global, ignorando `bg-transparent`). Quanto mais distantes as duas cores, mais visivel o defeito.

### Padrao global — DARK PREMIUM

**Estrutura:** um unico elemento visual. O `<input>` (nunca um `<div>` ao redor dele) carrega altura, borda, `border-radius`, fundo e padding. A lupa (`lucide-react` `Search`) fica dentro de um wrapper `relative` como irmao do `<input>`, posicionada com `absolute left-3` (compacto, dropdown) ou `left-4` (campo standalone maior), `top-1/2 -translate-y-1/2`, `pointer-events-none`; o `<input>` recebe `pl-9` (compacto) ou `pl-11` (standalone) para abrir espaco. **Proibido:** um `<div>` com `bg-*`/`border`/`rounded-*` proprios envolvendo um `<input>` com `bg-transparent`.

- Standalone (barra de busca isolada, maior): `h-12 w-full rounded-2xl border border-white/[0.08] bg-[#0D1926] pl-11 pr-4 text-sm font-semibold text-white/80 outline-none transition placeholder:text-white/25 hover:border-white/[0.15] focus:border-orange-400/40 focus:ring-2 focus:ring-orange-400/[0.08]`. Referencia: `PremiumSearch` (`app/questoes/page-client.tsx`), `RevisarSearch` (`app/questoes/revisar/page-client.tsx`), `FilterInput` (`app/admin/logs/page-client.tsx`), busca inline de `/admin/configuracoes/tentativas-cadastro`, `/admin/alunos`, `/admin/jornadas`, `/admin/eventos/[id]`.
- Compacto (busca interna de um dropdown/painel ja aberto): `h-9 w-full rounded-xl border border-white/[0.09] bg-white/[0.05] pl-9 pr-3 text-sm font-semibold text-white/80 outline-none placeholder:text-white/25 focus:border-orange-500/30 focus:ring-2 focus:ring-orange-500/[0.07]` — o fundo real acaba sendo o `#050b13` forcado pela regra global, o que e esperado e correto (nao lutar contra a regra global). Referencia: `SearchableSelect` (variante `dark`, apos a correcao desta entrega), `SubjectMultiSelect` (variante `dark`), `DarkMultiDropdown` (`app/simulados/[id]/editar/page-client.tsx`, apos a correcao desta entrega). Variantes sem lupa (`SubjectFilterDropdown`/`BoardFilterDropdown`/`OrgaoFilterDropdown`/`YearFilterDropdown`/`TopicFilterDropdown` em `app/questoes/page-client.tsx` e `app/questoes/revisar/page-client.tsx`) sao equivalentes validas — a lupa e opcional no compacto quando o campo ja esta obviamente dentro de um dropdown de filtro.
- Foco: sempre `focus:border-orange-400/40` (ou `/30` no compacto) + `focus:ring-2 focus:ring-orange-400/[0.08]` (ou `/[0.07]`) — o acento institucional laranja, nunca uma cor arbitraria.
- Placeholder: `placeholder:text-white/25` a `/30` conforme o restante do card.

### Padrao global — CLEAN PREMIUM

Mesma estrutura conceitual (`<input>` unico carrega a caixa; lupa `absolute` como irmao), adaptada ao tema claro:

- Compacto (dropdown/popover): `h-9 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-sm font-semibold text-slate-700 outline-none placeholder:text-slate-400 focus:border-orange-300 focus:bg-white focus:ring-4 focus:ring-orange-100`. Referencia: `SearchableSelect` (variante claro, apos esta correcao) e `SubjectMultiSelect` (variante claro), que ja seguia este padrao corretamente antes desta entrega — usado como referencia direta.
- O acento de foco no Clean e o mesmo par institucional (`orange-300`/`orange-100`) ja usado pelos demais campos `et-clean-field`; nao inventar uma nova cor de foco.

### Regra de reutilizacao

Novos campos de pesquisa devem reutilizar `SearchableSelect` (select pesquisavel de valor unico) ou `SubjectMultiSelect` (multi-select pesquisavel) quando o caso de uso for compativel; um novo dropdown de filtro (padrao `SubjectFilterDropdown`/`BoardFilterDropdown`) pode copiar a mesma estrutura de input localmente, pois esses componentes ja sao copias deliberadas do mesmo padrao (nao ha uma fabrica compartilhada unica para "dropdown de filtro multi-select com contagem", e criar uma agora seria refatoracao fora do escopo desta correcao). Um campo `PremiumInput` com `icon` (ex.: `/assuntos`, `/topicos`, `/bancas`, `/admin/ajuda`, `/professor/eventos/[id]`, `/admin/jornadas/[id]`, `/meu-perfil`) e uma variante valida e ja amplamente estabelecida para busca standalone com o icone ao lado do label (nao dentro do campo); nao foi unificada com o padrao "icone dentro do campo" nesta entrega porque `PremiumInput` e consumido com `icon` em mais de 25 arquivos, a maioria sem relacao com busca (icones de telefone, data, etc.) — mudar o comportamento do `icon` no componente compartilhado teria impacto amplo e nao mapeado, fora do risco aceitavel desta correcao.

### Correcoes aplicadas nesta entrega

- `app/components/ui/SearchableSelect.tsx`: removido o `<div>` com fundo proprio ao redor do campo de busca do dropdown, nas duas variantes (`dark` e claro); `<input>` passou a carregar a caixa completa (borda/radius/fundo/padding), com a lupa posicionada em `absolute`. Beneficia todos os 8+ consumidores com `dark` (`/topicos`, `/assuntos`, `/admin/alunos/[id]`, `/admin/raio-x-provas/[id]` e `/nova`, `/simulados`) e os consumidores em tema claro (Importador/Criador/Gerador de IA/editor de Simulado).
- `app/admin/eventos/[id]/ProfessorAssignmentPicker.tsx`: mesmo defeito (wrapper `bg-white/[0.04]` com `<input bg-transparent>` dentro) corrigido com a mesma tecnica.
- `app/simulados/[id]/editar/page-client.tsx`: dois pontos corrigidos — o campo "Buscar questão" do modal do Banco de Questões (wrapper `bg-[#0D1926]` com input transparente dentro, mesmo defeito de `PremiumSearch`/`RevisarSearch` porem implementado incorretamente aqui) e a busca interna de `DarkMultiDropdown` (wrapper `bg-black/30`, usado por Assuntos/Tópico/Banca/Dificuldade/Ano do mesmo modal), ambos convertidos para a mesma tecnica de caixa unica com lupa absoluta.

### Auditoria — nao alterado (sem defeito)

A varredura sistemica confirmou que a esmagadora maioria dos campos de busca do sistema ja segue o padrao correto de caixa unica (a "referencia" usada para corrigir os casos acima): `PremiumSearch`, `RevisarSearch`, `FilterInput` (`/admin/logs`), a busca de `/admin/alunos`, `/admin/alunos/[id]` (2 campos), `/admin/jornadas`, `/admin/jornadas/[id]` (campo "Buscar aluno" da listagem), `/admin/eventos/[id]` (2 campos de participantes), `/simulados`, `/questoes/importar` (busca de banca por questao), `/admin/raio-x-provas` (busca principal) e `/admin/raio-x-provas/nova` (combobox interno, com `bg-[#07111F]` no wrapper — cor praticamente identica ao `#050b13` forcado pela regra global, sem contraste visivel; mantido sem alteracao por nao haver defeito real, evitando substituicao mecanica). `EvaluatedTopicsInput` (campo de tópicos avaliados) ja usa a tecnica de caixa unica (sem wrapper) e nao foi alterado.

Nenhuma migration foi criada ou alterada nesta entrega. Nenhuma regra de busca, debounce, consulta, filtro, paginacao ou selecao foi alterada — apenas classes visuais dos campos identificados. Sem commit, push ou deploy.
