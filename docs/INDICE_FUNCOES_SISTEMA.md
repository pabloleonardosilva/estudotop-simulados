...
- `owl_help_enabled` é propagado ao cliente por três caminhos: query de `app/meus-simulados/[id]/page.tsx` (props iniciais), `GET /api/student/simulados/[id]` e `buildSimuladoSnapshot` em `POST /api/student/simulados/[id]/attempts`. `sanitizeAttempt` também retorna `owl_help_used_count` e `owl_help_data` para restaurar o estado ao retomar a tentativa.
- **Coruja voadora com regra de 10 segundos (atualizado 2026-07-16):** a coruja (`OwlHelpFlyingPrompt`, componente local de `page-client.tsx`, imagem oficial `/images/coruja-ajuda.jpg`) só aparece quando o aluno permanece mais de 10 segundos parado na mesma questão elegível, com tentativa ativa, ajudas restantes, questão sem ajuda anterior, resposta não bloqueada e nenhum modal aberto. Ela entra voando da direita em arco (framer-motion), com batida de asas simulada durante o voo (oscilação rápida e sutil de `scaleY`/`rotate`/`y`, sem deformar a arte), pousa na faixa livre entre o card da questão e a barra inferior de navegação (dentro da coluna principal, antes da coluna lateral — nunca sobre Caderno, Mapa da prova, Modo foco ou botões), passa a "respirar" (flutuação lenta) com sombra elíptica dinâmica sob os pés e só então exibe o balão de fala com rabinho apontado para o bico: "Você tem direito a X ajuda(s). Clique aqui!". Qualquer interação relevante (trocar questão, responder, tesourinha, caderno, anotação, abrir/fechar modal, usar a ajuda) esconde a coruja e reinicia a contagem. Sem overlay, sem `fixed inset-0`, sem backdrop-blur; a faixa expande com animação de altura para não deslocar a navegação bruscamente. Questões certo/errado nunca exibem a coruja.
- O clique abre modal de confirmação; confirmar chama `POST /api/student/simulados/[id]/attempts/[attemptId]/owl-help`, que valida no servidor: tentativa do aluno em andamento, simulado habilitado, limite (10% das questões, mínimo 1), questão pertencente à tentativa, questão não `true_false` e pelo menos duas alternativas erradas. O servidor escolhe as alternativas eliminadas e persiste `owl_help_used_count`/`owl_help_data`.
...
- O limite no cliente usa a mesma fórmula do backend (`getOwlHelpLimit`). A elegibilidade é revalidada no servidor — o cliente nunca decide quais alternativas são erradas.
...
- **Modal de preparação do feedback (2026-07-16):** quando a página de resultado é aberta com `attemptId` (tentativa recém-finalizada), o componente local `FeedbackPreparingModal` exibe o modal "Nossas corujas estão reunidas montando seu feedback" com contagem regressiva 5 → 0 (um passo/segundo) e fecha automaticamente no zero (saída de ~200 ms, sem clique, sem overlay residual). A contagem roda enquanto o resultado carrega por baixo; acessos sem `attemptId` (Meus Resultados, Jornada, links diretos) não exibem o modal.
...
**Regra permanente:** *o e-mail de boas-vindas é disparado pelo EVENTO de aprovação inicial do cadastro. Ele nunca deve ser disparado apenas porque o status do aluno mudou para `active`.*
...
- **Paginação**: prev/next + page indicator (orange) + select "N por página" com `[color-scheme:dark]`
