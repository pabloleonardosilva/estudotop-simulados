...
- **Integração Hotmart V1:** `/admin/configuracoes/hotmart`, `POST /api/webhooks/hotmart`, APIs em `/api/admin/hotmart`, processamento/e-mail/histórico em `app/lib/server/hotmart/` e migrations `20260828110000_create_hotmart_integration.sql` e `20260830120000_complete_hotmart_admin_workflows.sql`, ambas **APLICADAS** e estruturalmente validadas. Detalhes e pendências externas em `docs/Sprint-integracao-hotmart.md`.
- **Consistência administrativa Hotmart:** as RPCs de extensão concorrente, incremento de reprocessamento, resolução “manter separados” e claim/finalização recuperável de e-mails foram homologadas funcionalmente no banco real. A bateria interna terminou com 35/35 cenários aprovados e baseline preservado após a limpeza das fixtures.
- **Idempotência final Hotmart (2026-08-30):** retries de primeiro acesso consultam primeiro o token determinístico da transação/aluno; token usado encerra o claim local sem novo e-mail, e entrega ainda não usada mantém payload e idempotency key. Estados financeiros repetidos, inclusive `PURCHASE_REFUNDED` com novo `event.id`, permanecem no ledger externo sem reaplicar bloqueio, timestamp ou histórico funcional. O escopo interno está concluído; OAuth, webhook/refund sandbox ou real e envio real por Resend permanecem pendentes por dependência externa.
- **Readiness externo Hotmart (2026-08-30):** homologação externa parcial e interrompida antes de qualquer autenticação. HOTTOK e credenciais OAuth estão ausentes; o POST sem HOTTOK em `https://simulados.estudotop.com.br/api/webhooks/hotmart` retornou 404, portanto o código atual ainda depende de deploy autorizado. A conferência oficial também identificou pendências de compatibilidade em timestamps Unix em milissegundos, credencial OAuth Basic e separação segura entre refund sandbox/produção. Nenhuma chamada OAuth/refund, entrega Resend ou alteração comercial foi realizada.
...
- **Mapa funcional atual da integração Hotmart (homologação, 2026-09-14, branch `hotmart-homologacao`):** Admin `/admin/configuracoes/hotmart` (`app/admin/configuracoes/hotmart/page-client.tsx`) concentra catálogo Sandbox, catálogo Production (read-only), lookup por UCODE, Produtos vinculados, Transações, Pendências, Compras em duplicidade, Cadastros duplicados e Histórico. UCODE Hotmart é validado como 8-4-4-4-12 hexadecimal case-insensitive (`HOTMART_UCODE_PATTERN` em `app/lib/server/hotmart/products.ts`, sem exigir nibble de versão/variante RFC 4122), sempre normalizado para minúsculas antes de comparar/gravar. Sandbox usa `HOTMART_CLIENT_ID/CLIENT_SECRET/BASIC_TOKEN` + `HOTMART_ENVIRONMENT` (`app/lib/server/hotmart/config.ts`, `refund.ts`, `products.ts`); catálogo/lookup: `GET /api/admin/hotmart/products` e `GET /api/admin/hotmart/products/lookup`. Production (somente leitura, nunca usada por webhook/refund/processor) usa credenciais isoladas `HOTMART_PRODUCTION_CLIENT_ID/CLIENT_SECRET/BASIC_TOKEN`, sem depender de `HOTMART_ENVIRONMENT`, em módulo próprio `app/lib/server/hotmart/productionCatalog.ts`; catálogo/lookup: `GET /api/admin/hotmart/products/production` e `GET /api/admin/hotmart/products/production/lookup`, sempre contra `developers.hotmart.com`. O formulário "Vincular produto" tem seletor explícito de origem (Sandbox/Produção) que decide qual lookup o botão "Buscar produto" chama; "Usar este produto" nas duas listagens preenche UCODE/nome e a origem correspondente sem nova consulta. O card de transação (reaproveitado por Transações, Pendências e Compras em duplicidade via `renderTransactionCard`) mostra Produto/UCODE com copiar, Compra (data comercial, valor na moeda real quando `amount`/`currency` existirem em `hotmart_transactions` — moeda nunca é presumida como "BRL" —, situação traduzida), Comprador, Acesso no EstudoTOP com 4 estados derivados só do mapping do produto por UCODE (nunca de `processing_status`): Vinculado (mapping ativo + destino resolvido), Destino indisponível (mapping ativo, destino não resolvido), Vínculo inativo (mapping existe mas não está ativo), Produto não vinculado + botão Vincular (nenhum mapping) — reaproveitando o modal já existente de Pendências; "Vincular e reprocessar" só aparece quando o `processing_status` é elegível para reprocessamento. Transação/situação interna traduzida, sem exibir códigos crus. Destino de mapping só suporta `jornada`/`event` (`hotmart_product_mappings_destination_check`); Simulado direto não é suportado sem migration. Detalhes completos em `docs/Sprint-integracao-hotmart.md`.
