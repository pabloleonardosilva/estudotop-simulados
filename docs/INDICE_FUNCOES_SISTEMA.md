...
- **Cooldown baseado em envio confirmado (2026-08-22):** em `POST /api/events/[slug]`, o Resend é chamado **antes** de qualquer escrita em `simulado_event_join_intents` — a intent só é gravada (delete da anterior + insert) depois de o provider confirmar sucesso. Isso elimina a janela em que uma falha silenciosa do Resend deixava uma intent "fantasma" fazendo o cooldown de 60s responder sucesso sem reenviar nada. Mensagem do cooldown real informa explicitamente o tempo restante, distinta da mensagem de sucesso. A exclusão de aluno (`app/api/admin/students/[id]/route.ts` e `.../hard-delete/route.ts`) passou a remover as intents **não consumidas** do e-mail excluído (intents consumidas são preservadas como histórico de auditoria, sem risco de bloquear recadastro).
...
- **Menu condicional para aluno exclusivamente de Evento (2026-08-21):** `GET /api/student/nav-access` deriva dinamicamente `has_jornadas` (existe `student_jornadas` com `status != 'cancelled'`) e `has_event_origin` (`students.origin_event_id is not null`) — sem gravar nenhuma flag nova. `AuthContext` busca essa rota uma única vez por sessão de aluno e expõe `studentNavAccess` via `useAuth()`, fonte única consumida por `Header.tsx` (menu superior), `Sidebar.tsx` (menu lateral/hambúrguer, `SidebarContent`) e `AppShell.tsx`. Item "Jornadas" só aparece com `hasJornadas`; item "Simulados" só aparece sem `hasEventOrigin` (os dois são independentes — Jornada não concede acesso ao módulo geral de Simulados). "Meus Eventos" no menu lateral continua regido pela checagem pré-existente de `hasEvents` (`/api/student/events`), não alterada.
- **Tutorial das Corujas suprimido para aluno exclusivamente de Evento (2026-08-21):** `StudentJourneyExplainerModal` (aberto por `AppShell.tsx`) não abre automaticamente quando `studentNavAccess.hasEventOrigin && !studentNavAccess.hasJornadas`. Supressão apenas contextual: nada é persistido como "já visto"; o contador de `localStorage` (limite de 10 logins) só avança fora desse estado, então o tutorial volta a funcionar normalmente se o aluno receber uma Jornada depois.
