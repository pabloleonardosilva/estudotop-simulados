...
- Interface: `HelpCenterModal` é claro e compacto; `/admin/ajuda` permanece dark premium e abre conversa, dados do aluno, contexto técnico e timeline sob demanda. O badge no `Sidebar` continua significando tickets abertos; a bolinha da fila significa `admin_seen_at = null`.
...
- A migration desta evolução não foi executada durante a Sprint; o fluxo depende de aplicação manual posterior e homologação ponta a ponta.
