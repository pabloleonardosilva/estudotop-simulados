...
- **Gestão e auditoria de uso em `/topicos` (2026-08-14):** a contagem e a proteção de exclusão percorrem todas as páginas da tabela `questions`, sem depender do limite padrão de uma consulta Supabase. Clicar no nome ou na contagem abre a relação completa de questões vinculadas, incluindo o status que explica itens fora da visão padrão do banco (revisão, fila, arquivada etc.) e acesso à edição da questão. Tópicos sem vínculos exibem exclusão direta com confirmação; a API continua recalculando o uso no momento da exclusão. A edição de questão mantém validação no cliente e no servidor que impede salvar `evaluated_topics` vazio.
