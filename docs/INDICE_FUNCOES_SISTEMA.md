...
- **Imagem configurável do Evento e card do aluno análogo ao card de Jornada (2026-08-25):** `simulado_events.cover_key` (novo, `text null`, migration `20260825070000_add_simulado_event_cover_key.sql`, não executada) — chave de um catálogo controlado (`EVENT_COVERS`, novo arquivo `app/admin/eventos/utils.ts`, isolado de `app/admin/jornadas/utils.ts`, sem dependência funcional entre os dois; imagens inicialmente reaproveitadas de `public/jornadas/categories/*.webp`). Admin escolhe a capa por cards visuais (mesmo padrão do seletor de categoria de Jornada, reimplementado localmente) em `/admin/eventos` (criação, formulário inline) e `/admin/eventos/[id]` (edição, bloqueada quando arquivado pela trava já existente). `POST /api/admin/events` e `PATCH /api/admin/events/[id]` validam `cover_key` contra uma lista fechada de chaves (`EVENT_COVER_KEYS`, mesmo padrão local já usado por `app/api/admin/jornadas/route.ts`) — nunca aceita URL/caminho arbitrário. Duplicação (`action: "duplicate"`) copia `cover_key` do original. `cover_key` nulo (todo Evento existente antes desta entrega) resolve para a capa `administrativo` — nenhum Evento quebra. `GET /api/student/events` passou a trazer `cover_key`. **Card de `/meus-eventos` redesenhado** (único arquivo de UI do aluno alterado) — estrutura análoga ao `JornadaCard` de `app/minhas-jornadas/page-client.tsx`: imagem de capa no topo com tag "Evento de Simulado", badge de status global (`student-status-badge`, reaproveitando `student-status-active`/nova `student-status-scheduled`/`student-status-expired` para closed-archived), professor(es) discreto, grade 2×2 de mini-cards (`Início`/`Término`/`Sua situação`/`Tentativas`, classes `student-metric-card` já existentes) e CTAs com `student-button-primary`/`student-button-secondary`. Único CSS novo: `.student-status-scheduled` em `app/globals.css` (aditivo, Jornada não tem esse estado). Toda a lógica de tentativas/situação/CTA/modal "Refazer simulado" da entrada acima foi preservada, não recriada — apenas re-skinada. `Sidebar.tsx`/`Header.tsx`/`AppShell.tsx` e todo o módulo de Jornadas (`app/admin/jornadas/**`, `app/minhas-jornadas/**`) não foram tocados. Detalhe do Evento (`/meus-eventos/[id]`) e listagem/detalhe administrativos não foram redesenhados nesta entrega (fora do pedido).
...
- **Compatibilidade:** Jornada mantém `category`; Evento mantém `cover_key`; ambos continuam como fallback. Evento sem banner mantém o hero atual. Os assets legados em `public/jornadas/categories/` não foram removidos.
- **Bootstrap:** `scripts/bootstrap-system-images.mjs`, explícito e idempotente, cria cópias independentes dos quatro WebP legados para Jornada e Evento e associa somente FKs nulas.
