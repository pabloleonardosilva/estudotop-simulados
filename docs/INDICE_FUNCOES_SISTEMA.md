...
- **Reentrada com intenção pendente (2026-08-28):** `POST /api/events/[slug]` responde explicitamente `confirmation_email_sent` quando o Resend confirma um novo envio e `confirmation_pending` quando uma solicitação válida está em cooldown ou vence uma corrida concorrente. `/evento/[slug]` reconhece ambos os estados e sempre abre a tela **Confira seu e-mail**, preservando reCAPTCHA, cooldown, anti-enumeração, reenvio e troca de e-mail.
