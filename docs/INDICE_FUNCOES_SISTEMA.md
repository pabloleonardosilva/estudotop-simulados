...
### 10.X Tentativas incompletas de cadastro público (2026-09-10)
...
**Tabela:** `public.student_registration_attempts` (`supabase/migrations/20260910100000_student_registration_attempts.sql` — criada, **não executada**). Uma linha por `email_normalized` (`lower(btrim(email))`) enquanto `status <> 'completed'` (índice único parcial `student_registration_attempts_open_email_unique`, alvo do `ON CONFLICT` das funções abaixo — garante dedupe mesmo sob concorrência, nunca depende de `SELECT` seguido de `INSERT`). Campos: `email`/`email_normalized`, `full_name`, `phone`/`phone_normalized`, `status` (`open|contacted|ignored|completed` — decisão administrativa), `stage` (`confirmation_sent|confirmation_confirmed|account_creation_failed|completed` — etapa técnica real do fluxo, nunca misturada com `status`), `source` (`public_signup|event_signup`) + `source_context_id` (event_id), `first_started_at` (preservado em toda atualização), `last_activity_at`, `confirmation_requested_at`/`confirmation_confirmed_at`/`completed_at`/`ignored_at`, `attempt_count` (submissões voluntárias reais — nunca conta reenvio automático por código errado), `confirmation_send_count` (todo código enviado, incluindo reenvio automático), `last_failure_code`/`last_failure_message` (código interno sanitizado de `StudentAccountErrorCode` — nunca SQL bruto/stack/payload), `admin_contacted_at`/`admin_contacted_by`/`admin_notes`. **Nunca armazena senha, OTP, código de confirmação ou CPF** (minimização — o objetivo operacional é nome/e-mail/telefone). RLS habilitado, `anon`/`authenticated` sem nenhum privilégio — acesso exclusivo via API admin server-side.

**5 funções SQL** (mesmo padrão `security invoker set search_path=''`, revogadas de `public/anon/authenticated`, concedidas só a `service_role` — nunca chamáveis do browser, já auditado no Sprint da engine de tentativas): `upsert_student_registration_attempt` (upsert atômico por `email_normalized`, chamado só quando o servidor efetivamente aceita o cadastro — código gerado e e-mail enviado com sucesso), `touch_student_registration_attempt_resend` (reenvio automático por código incorreto — só soma `confirmation_send_count`, nunca `attempt_count`, nunca muda `stage`), `mark_student_registration_attempt_confirmed` (código validado, antes de tentar criar a conta), `complete_student_registration_attempt` (conta constituída — `auth.users`+`profiles`+`students` — único ponto que marca `completed`, nunca reaberto depois), `mark_student_registration_attempt_failed` (`createStudentAccount` falhou, com rollback já aplicado pelo próprio serviço — a tentativa permanece recuperável).

**Serviço central:** `lib/server/studentRegistrationAttemptService.ts` (`startOrTouchRegistrationAttempt`, `touchRegistrationAttemptResend`, `markRegistrationAttemptConfirmed`, `completeRegistrationAttempt`, `markRegistrationAttemptFailed`) — cada função chama o RPC correspondente e nunca lança: falha aqui é registrada em `system_error_logs` e absorvida, o tracking é secundário e nunca pode quebrar o cadastro real.
...
- `app/api/auth/register/route.ts` — chama `startOrTouchRegistrationAttempt` só depois do código gerado e do e-mail enviado com sucesso (logo antes do `return ok:true`). `source = event ? "event_signup" : "public_signup"`.
- `app/api/auth/confirm-registration/route.ts` — três pontos: (1) no reenvio automático por código incorreto, chama `touchRegistrationAttemptResend`; (2) logo após o código ser validado (`claimedConfirmation`), chama `markRegistrationAttemptConfirmed`; (3) dentro do `try` de `createStudentAccount`, sucesso chama `completeRegistrationAttempt` (mesmo em fluxo de Evento — antes dos passos específicos de Evento que podem falhar depois, pois a conta já está constituída nesse ponto), e o `catch` chama `markRegistrationAttemptFailed` reaproveitando o mesmo `studentAccountErrorResponse(error, true)` já calculado para a resposta HTTP (nunca uma segunda sanitização).
- **Nunca chamado** por `/api/admin/students/create` (cadastro manual do admin não gera tentativa) nem por `lib/server/studentAccountService.ts` (serviço compartilhado por ambos os fluxos — o tracking fica só nos dois endpoints do cadastro público, nunca dentro do serviço).
...
**Diferença incomplete × pending:** `students.status = 'pending'` (aguardando aprovação administrativa) é cadastro **concluído** (conta íntegra já existe) — nunca aparece na central. Só aparece aqui quem ainda não chegou a esse ponto.

**APIs admin:** `GET /api/admin/registration-attempts` (`requireAdmin`, paginação server-side — padrão 25, máximo 100 —, filtros `search`/`status`/`stage`/`period`, exclusão da identidade protegida, e métricas globais `open`/`last24h`/`contacted`/`completed` via 4 consultas `count:"exact", head:true` em paralelo, independentes da página atual). `PATCH /api/admin/registration-attempts/[id]` (allowlist de ações `contact|ignore|reopen|note` — nunca permite editar e-mail/nome/telefone/etapa; bloqueado se `status = completed`; grava `logAdminAction`). `DELETE /api/admin/registration-attempts/[id]` (remove só o registro de acompanhamento — nunca `auth.users`/`profiles`/`students`/`student_registration_confirmations`; `logAdminAction`).

**Tela:** `/admin/configuracoes/tentativas-cadastro` (`page.tsx` com `requireAdminPage`, `page-client.tsx` dark premium — mesma linguagem de `/admin/logs`: hero, 4 cards de métrica, painel de filtros, tabela com paginação, modal de detalhe `size="wide"` com linha do tempo derivada dos próprios timestamps, copiar e-mail/telefone, link `wa.me/<telefone_normalizado>`, observação administrativa, ignorar/reabrir/marcar contatado/excluir com confirmação). Link adicionado em `app/components/Sidebar.tsx` (**arquivo protegido** — alteração mínima: um novo `NavLink` dentro do grupo "Configurações" já existente, nenhum item removido ou reordenado) com badge de contagem de tentativas em aberto (reaproveita o padrão já existente de `openHelpMessagesCount`, `adminFetch` + `setInterval` de 30s — sem realtime novo).
...
**Preservado integralmente:** `student_registration_confirmations` (responsabilidade técnica de código/expiração inalterada); `createStudentAccount`/`studentAccountService.ts`/`studentAccountRepair.ts` (nenhuma linha tocada); fluxo de cadastro público (`app/cadastro/page.tsx`, `app/cadastro/confirmar/page.tsx` — este último é do fluxo de convite, não do código de 6 dígitos, não relacionado); fluxo de Evento; nenhum envio automático de WhatsApp/SMS/e-mail comercial foi criado — a central é 100% manual.
