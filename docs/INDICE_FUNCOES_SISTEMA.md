...
- **Padronização dark de Configurações (2026-08-16):** `/assuntos`, `/disciplinas`, `/bancas` e `/bancas/importar` adotam a mesma composição visual de `/topicos`: hero institucional, coluna compacta de criação ou filtros, painel principal glass, filtros agrupados, botões primários laranja, estados vazios e cards internos dark responsivos. O importador de bancas mantém a entrada em massa por linhas e destaca equivalências antes do envio. Os modais de confirmação, ativação/inativação, feedback e exclusão dessas páginas usam explicitamente o `PremiumModal` com `theme="dark"` e ações dark. APIs, validações, normalização, vínculos e regras de exclusão permanecem inalterados.
...
- **Modal "Cronograma do Evento" (2026-08-24):** novo modal em `app/admin/alunos/[id]/page-client.tsx`, visualmente equivalente ao modal "Cronograma individual" da Jornada mas com estado próprio e isolado (`scheduleModalEvent`, `eventScheduleProcessingId`, `eventAttemptDrafts`, `resetEventAttemptsTarget` — nenhum estado de Jornada foi reaproveitado ou alterado). Mostra o simulado vinculado ao Evento, situação, tentativas válidas/total, resultado (liberado/aguardando) e permite ajustar o número de tentativas do aluno naquele Evento. Digitar `0` abre confirmação destrutiva própria (texto explícito de que tentativas de Jornada/avulsas do mesmo simulado não são afetadas) antes de zerar.
- **`PATCH /api/admin/events/[id]/participants/[studentId]` (2026-08-24, novo):** ação `set_attempts` (`{ action: "set_attempts", attempts: number }`). Reaproveita o padrão de `PATCH /api/admin/student-jornadas/.../simulados/[id]` (criação de tentativas placeholder `status: "abandoned"` quando `attempts` aumenta, toggle de `counts_toward_limit` nas mais antigas) mas com **duas funções novas e isoladas** (`setEventParticipantAttemptsCount`, `resetEventParticipantHistory`) que filtram sempre por `event_participant_id` + `event_id` + `student_id` — nunca reaproveitou as funções de Jornada, que operam por `student_id + simulado_id` em todos os contextos (o que apagaria/recontaria tentativas de Jornada e avulsas do mesmo Simulado). Zerar (`attempts: 0`) apaga apenas as tentativas escopadas ao Evento/participante (cascata de FK cuida de `simulado_answers`/`simulado_results`/`topcoin_earnings`) e limpa `representative_attempt_id`/`result_released_at` do participante. **Decisão documentada:** TopCoins não são recalculados nesta rota — a função de Jornada equivalente (`resyncTopCoinEarnings`) opera por `student_id + simulado_id` em todos os contextos e renumeraria tentativas de Jornada/avulso ao ser chamada aqui; tentativas apagadas já perdem seus ganhos por `ON DELETE CASCADE`, e tentativas placeholder nascem `abandoned` (nunca geram TopCoins), então não há inconsistência introduzida por essa omissão deliberada.
