...
- **Responsabilidade:** visão geral consolidada, ranking oficial por acertos/tempo, detalhes individuais, revisão pedagógica com revelação controlada e atualização silenciosa a cada dez segundos.
- **Classificação dos participantes (2026-08-26):** a aba oferece os modos locais `Por nota` (padrão, baseado no ranking oficial existente) e `Ordem alfabética` (`localeCompare` em `pt-BR`). A tabela resumida mantém apenas posição, aluno, situação, nota e ação `Ver`; no modo por nota, os três primeiros recebem troféus Lucide com tons de ouro, prata e bronze. O modal individual e seus dados permanecem inalterados.
