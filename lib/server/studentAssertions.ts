...
// Bloqueio comercial Hotmart (política aprovada na Fase 4B): impede novas
// ações interativas do aluno sobre uma tentativa cujo Evento/Jornada de
// origem está com acesso comercial inativo (atraso, reembolso, chargeback,
// cancelamento). Padrão desenhado no primeiro commit da integração Hotmart
// (b682ec1) e nunca alterado desde então. Nunca usado em
// simuladoAttemptCompletion.ts nem no job de timeout/reconciliação
// histórica — essas finalizações automáticas do sistema não são "ação
// interativa do aluno" e devem continuar funcionando mesmo com o acesso
// bloqueado (senão a tentativa fica presa em in_progress para sempre).
// Standalone (sem event_participant_id nem student_jornada_simulado_id)
// nunca é bloqueado aqui, porque nenhuma das duas condições se aplica.
