  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        // A intenção já foi validada com segurança pelo backend (token de uso
        // único hash-comparado). Comparamos o e-mail dela com uma sessão
        // porventura já aberta neste navegador — nunca o contrário — para
        // impedir que uma sessão antiga sequestre o cadastro/login de outra
        // identidade.
        const { data: auth } = await supabase.auth.getSession();
