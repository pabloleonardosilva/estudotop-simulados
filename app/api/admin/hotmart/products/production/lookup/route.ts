...
import { HOTMART_UCODE_PATTERN } from "@/app/lib/server/hotmart/products";
import { HotmartProductionCatalogError, lookupHotmartProductionProductByUcode } from "@/app/lib/server/hotmart/productionCatalog";

function lookupErrorResponse(error: HotmartProductionCatalogError) {
...
// Lookup individual, read-only, do catálogo REAL (produção) da Hotmart por UCODE.
// Usa exclusivamente HOTMART_PRODUCTION_* (via productionCatalog.ts) — nunca credenciais Sandbox,
// nunca depende de HOTMART_ENVIRONMENT. Não cria mapping, não altera banco, não escreve na Hotmart.
export async function GET(request: Request) {
  ...
  try {
    const product = await lookupHotmartProductionProductByUcode(ucode);
    return NextResponse.json({ ok: true, message: "Produto encontrado.", product });
  } catch (error) {
    return lookupErrorResponse(error instanceof HotmartProductionCatalogError ? error : new HotmartProductionCatalogError("unavailable"));
  }
}
