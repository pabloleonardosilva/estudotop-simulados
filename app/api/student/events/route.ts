...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from("simulado_event_participants").select("id,joined_at,representative_attempt_id,result_released_at,simulado_events:event_id(id,name,status,starts_at,ends_at,simulado_id,result_policy,simulados:simulado_id(title),simulado_event_professors(professors:professor_id(name)))").eq("student_id", student.id).order("joined_at", { ascending: false });
