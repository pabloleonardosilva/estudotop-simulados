...
export async function GET(request: Request) {
  ...
  const events = (data || []).map((row) => { const event = row.simulado_events as unknown as Record<string, unknown> & { id: string; status: string; starts_at: string; ends_at: string; simulado_id: string | null; card_image?: { storage_path?: string } | null }; return { ...row, attempts: (attempts || []).filter((attempt) => attempt.event_id === event.id && attempt.simulado_id === event.simulado_id), simulado_events: { ...event, card_image_url: systemImageUrl(event.card_image?.storage_path), effective_status: effectiveEventStatus(event) } }; });
