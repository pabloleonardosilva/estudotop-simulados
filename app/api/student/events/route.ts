...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from("simulado_event_participants").select("id,joined_at,representative_attempt_id,result_released_at,access_status,access_origin,commercial_block_reason,simulado_events:event_id(id,name,max_attempts,status,starts_at,ends_at,simulado_id,result_policy,cover_key,card_image:card_image_id(storage_path),simulados:simulado_id(title),simulado_event_professors(professors:professor_id(name)))").eq("student_id", student.id).order("joined_at", { ascending: false });
