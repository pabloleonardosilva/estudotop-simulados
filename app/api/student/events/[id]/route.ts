...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: participant } = await supabase.from("simulado_event_participants").select("id,representative_attempt_id,result_released_at,simulado_events:event_id(id,name,status,starts_at,ends_at,started_at,simulado_id,result_policy,simulados:simulado_id(id,title,max_attempts,time_limit_minutes,focus_violation_limit),simulado_event_professors(professors:professor_id(name)))").eq("event_id", id).eq("student_id", student.id).maybeSingle();
