...
export async function GET(
  ...
  // Resultado real: primeira tentativa concluída que ainda conta no limite.
  const { data: attempt } = await supabase
    .from("simulado_attempts")
    .select("id, status, time_spent_seconds, submitted_at, disqualified_at, disqualification_reason, tab_switch_count, focus_violation_count, inactivity_event_count, scissors_used_question_ids, owl_help_used_count")
  ...
  void logStudentActivity({ studentId: student.id, action: "student.result.viewed", entityType: "attempt", entityId: attempt.id, request, metadata: { simulado_id: simuladoId } });
