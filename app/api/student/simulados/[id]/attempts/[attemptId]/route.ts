...
export async function POST(
  ...
  const requestedViolationNumber = Math.max(1, Math.floor(body.violation_number || 1));
  ...
  const nextViolationCount = Math.max(
    (attempt.focus_violation_count || 0) + 1,
    requestedViolationNumber,
  );
  ...
  if (updateError) {
    void logSystemError({ source: "api.student.attempt_focus_violation", error: updateError, request, metadata: { attempt_id: attemptId } });
    return NextResponse.json(
