...
export const FIRST_ACCESS_COOKIE = "estudotop_first_access";
export const FIRST_ACCESS_COOKIE_PATH = "/api/auth/first-access";

export async function GET() {
  const cookieStore = await cookies();
  const token = cookieStore.get(FIRST_ACCESS_COOKIE)?.value || "";
  if (!token) return NextResponse.json({ ok: false, message: "Contexto de primeiro acesso ausente ou expirado." }, { status: 404 });
  ...
  const { data: confirmation } = await supabase
    ...
    .in("token_hash", emailActionTokenHashCandidates(token))
    .is("used_at", null)
    .gt("expires_at", new Date().toISOString())
    .maybeSingle();

  const eventId = typeof confirmation?.metadata?.event_id === "string" ? confirmation.metadata.event_id : null;
  if (!confirmation || !eventId) return NextResponse.json({ ok: false, message: "Contexto de primeiro acesso ausente ou expirado." }, { status: 404 });
  const { data: event } = await supabase.from("simulado_events").select("public_slug").eq("id", eventId).maybeSingle();
  if (!event?.public_slug) return NextResponse.json({ ok: false, message: "Contexto de primeiro acesso ausente ou expirado." }, { status: 404 });
  return NextResponse.json({ ok: true, message: "Contexto de primeiro acesso recuperado.", email: confirmation.email, event_id: eventId, event_slug: event.public_slug });
}

export async function POST(request: Request) {
  try {
    const { token: bodyToken, password, confirmPassword } = (await request.json()) as FirstAccessPayload;
    const cookieStore = await cookies();
    const token = bodyToken || cookieStore.get(FIRST_ACCESS_COOKIE)?.value || "";
    ...
    if (profileError) {
      ...
      return NextResponse.json({ ok: false, code: "PASSWORD_UPDATED_POST_STEP_FAILED", password_updated: true, message: "Sua senha foi atualizada, mas não foi possível concluir a liberação do acesso. Tente concluir novamente." }, { status: 500 });
    }
    ...
    if (!preserveAccountStatus) {
      ...
      if (studentError) {
        ...
        return NextResponse.json({ ok: false, code: "PASSWORD_UPDATED_POST_STEP_FAILED", password_updated: true, message: "Sua senha foi atualizada, mas não foi possível concluir a ativação do acesso. Tente concluir novamente." }, { status: 500 });
      }
    }
    ...
    if (tokenUpdateError) {
      ...
      return NextResponse.json({ ok: false, code: "PASSWORD_UPDATED_POST_STEP_FAILED", password_updated: true, message: "Sua senha foi atualizada, mas o acesso não pôde ser finalizado. Tente concluir novamente." }, { status: 500 });
    }
    ...
    response.cookies.set(FIRST_ACCESS_COOKIE, "", { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: FIRST_ACCESS_COOKIE_PATH, maxAge: 0 });
    return response;
  } catch (error) {
