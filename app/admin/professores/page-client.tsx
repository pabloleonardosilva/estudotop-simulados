...
export default function ProfessoresAdminClient() {
  ...
  return <main className="min-h-full bg-[#050b14] px-4 py-8 text-white"><div className="mx-auto max-w-6xl"><div className="flex justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-orange-400">Gestão</p><h1 className="mt-2 text-3xl font-black">Professores</h1></div><PremiumButton onClick={() => setOpen((value) => !value)}><Plus size={17} /> Novo professor</PremiumButton></div>{message && <p className="mt-4 text-orange-200">{message}</p>}{open && <form onSubmit={submit} className="mt-6 grid gap-4 rounded-2xl border border-white/10 bg-white/[0.05] p-6 md:grid-cols-3"><PremiumInput name="name" label="Nome" required /><PremiumInput name="email" label="E-mail" type="email" required /><PremiumInput name="phone" label="WhatsApp" /><PremiumButton type="submit" disabled={saving}>{saving && <Loader2 size={16} className="animate-spin" />} Cadastrar</PremiumButton></form>}<div className="mt-8 grid gap-4 md:grid-cols-2">{rows.map((row) => <article key={row.id} className="rounded-2xl border border-white/10 bg-white/[0.05] p-5"><GraduationCap className="text-orange-400" /><h2 className="mt-3 font-black">{row.name}</h2><p className="mt-1 text-sm text-slate-400">{row.email} · {row.status}</p></article>)}</div></div></main>;
}
