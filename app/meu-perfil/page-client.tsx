...
import { BarChart3, CalendarDays, Check, CheckCircle2, ChevronRight, ClipboardCheck, Clock3, Database, Flag, IdCard, Images, LifeBuoy, Loader2, LockKeyhole, Mail, Map, Pencil, Phone, ShieldCheck, Sparkles, Target, Trash2, User, X } from "lucide-react";
...
const card = "rounded-[24px] border border-slate-200/75 bg-white p-5 shadow-[0_18px_55px_rgba(15,23,42,0.065)] md:p-6";
...
export default function MeuPerfilClient() {
  ...
  async function selectAvatar(avatarId: string) {
    ...
    try {
      const response = await authFetch("/api/profile/avatar", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ avatar_id: avatarId }) });
      const json = await response.json().catch(() => ({}));
  ...
  return (
    <div className="min-h-full bg-gradient-to-br from-[#fffaf4] via-[#fbfaf8] to-[#f4f7fa] px-4 py-6 md:px-7 md:py-8 lg:px-10">
      <div className="mx-auto max-w-[1200px] space-y-6">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <section className={card}><CardTitle icon={<User size={18} />} title="Dados pessoais" text="Mantenha seus dados atualizados para identificarmos corretamente sua conta." /><div className="mt-6 space-y-4"><PremiumInput label="Nome completo" icon={<User size={15} />} value={name} maxLength={120} className="rounded-[14px]" onChange={(event: ChangeEvent<HTMLInputElement>) => setName(event.target.value)} /><PremiumInput label="Telefone" icon={<Phone size={15} />} value={phone} maxLength={25} inputMode="tel" className="rounded-[14px]" onChange={(event: ChangeEvent<HTMLInputElement>) => setPhone(event.target.value)} />{data.profile.cpf && <PremiumInput label="CPF" icon={<IdCard size={15} />} value={formatCpf(data.profile.cpf)} readOnly className="cursor-not-allowed rounded-[14px] bg-slate-50 text-slate-500" />}</div><PremiumButton className="mt-6 min-h-11 rounded-[14px]" onClick={savePersonal} disabled={!changed || saving}>{saving && <Loader2 size={16} className="animate-spin" />} Salvar alterações</PremiumButton></section>
        ...
        <section className={card}><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><CardTitle icon={<Target size={18} />} title="Meus interesses" text="Escolha os concursos que fazem parte dos seus objetivos de estudo." /><PremiumButton variant="secondary" className="min-h-10 rounded-[14px]" onClick={() => setInterestOpen(true)}><Pencil size={15} /> Editar interesses</PremiumButton></div><div className="mt-6 flex min-h-12 flex-wrap items-center gap-2.5">{data.interests.saved_names.length ? data.interests.saved_names.map((interest) => <span key={interest} className="rounded-full border border-orange-200 bg-gradient-to-r from-orange-50 to-amber-50 px-3.5 py-2 text-xs font-bold text-orange-700 shadow-sm">{interest}</span>) : <p className="text-sm text-slate-500">Nenhum interesse cadastrado.</p>}</div></section>

        <div className="grid gap-5 lg:grid-cols-2"><section className={card}><CardTitle icon={<BarChart3 size={18} />} title="Minha trajetória" text="Um resumo simples da sua atividade no EstudoTOP." /><div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2"><Metric icon={<ClipboardCheck size={18} />} value={data.trajectory.completed_simulados} label="Simulados concluídos" /><Metric icon={<CheckCircle2 size={18} />} value={data.trajectory.answered_questions} label="Questões respondidas" /></div><TextLink href="/meus-resultados">Ver meus resultados</TextLink></section><section className={card}><CardTitle icon={<Map size={18} />} title="Minhas Jornadas" text="Acompanhe seus caminhos de preparação." /><div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2"><Metric icon={<Map size={18} />} value={data.journeys.active} label="Jornadas ativas" /><Metric icon={<Flag size={18} />} value={data.journeys.completed} label="Jornadas concluídas" /></div><TextLink href="/minhas-jornadas">Ver minhas Jornadas</TextLink></section></div>

        <section className={card}><CardTitle icon={<Mail size={18} />} title="Preferências" text="Comunicações essenciais da sua conta." /><div className="mt-5 flex flex-col gap-4 rounded-[18px] border border-slate-200 bg-slate-50/80 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold text-slate-800">Comunicações essenciais</p><p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">Avisos de segurança, recuperação de acesso e informações operacionais permanecem ativos. Ainda não existem preferências opcionais persistidas para configurar.</p></div><span className="inline-flex w-fit shrink-0 items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700"><CheckCircle2 size={14} /> Ativas</span></div></section>

        <div className="grid gap-5 lg:grid-cols-2"><section className={card}><CardTitle icon={<Database size={18} />} title="Privacidade e dados" text="Faça solicitações relacionadas aos seus dados e à sua conta." /><div className="mt-5 divide-y divide-slate-100 overflow-hidden rounded-[18px] border border-slate-200">{["Solicitar meus dados", "Solicitar correção de dados", "Solicitar exclusão da minha conta"].map((label) => <button key={label} type="button" onClick={openHelp} className="flex min-h-12 w-full items-center justify-between bg-white px-4 text-left text-sm font-semibold text-slate-700 transition hover:bg-orange-50/60 hover:text-orange-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-orange-400">{label}<ChevronRight size={16} /></button>)}</div><p className="mt-3 text-xs leading-5 text-slate-400">A Política de Privacidade será disponibilizada aqui quando houver uma rota pública oficial.</p></section><section className="relative overflow-hidden rounded-[24px] border border-orange-200 bg-gradient-to-br from-white via-orange-50/45 to-amber-50 p-5 shadow-[0_18px_55px_rgba(249,115,22,0.10)] md:p-6"><div className="pointer-events-none absolute -bottom-16 -right-10 h-44 w-44 rounded-full bg-orange-200/40 blur-3xl" /><div className="relative"><CardTitle icon={<LifeBuoy size={18} />} title="Precisa de ajuda?" text="Está com alguma dificuldade relacionada à sua conta ou aos seus dados?" /><p className="mt-5 text-sm leading-6 text-slate-600">Nossa equipe está pronta para orientar você pelo canal oficial da Central de Ajuda.</p><PremiumButton className="mt-5 min-h-11 rounded-[14px]" onClick={openHelp}><LifeBuoy size={16} /> Falar com o suporte</PremiumButton></div></section></div>
      </div>

      {avatarOpen && <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/65 p-3 backdrop-blur-md sm:p-5"><div role="dialog" aria-modal="true" aria-labelledby="avatar-title" className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[28px] border border-orange-200 bg-gradient-to-br from-white via-[#fffaf5] to-orange-50 shadow-[0_30px_100px_rgba(15,23,42,0.35)]"><div className="flex items-start justify-between gap-4 border-b border-orange-100 px-5 py-5 sm:px-7"><div className="flex gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[15px] bg-gradient-to-br from-orange-500 to-amber-400 text-white shadow-lg shadow-orange-200"><Sparkles size={20} /></span><div><p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-orange-600">Seu estilo</p><h2 id="avatar-title" className="mt-1 text-xl font-bold tracking-[-0.02em] text-slate-950 sm:text-2xl">Escolha seu avatar</h2><p className="mt-1 text-sm text-slate-500">Corujas EstudoTOP primeiro, seguidas por pessoas diversas.</p></div></div><button type="button" onClick={() => setAvatarOpen(false)} aria-label="Fechar seleção de avatares" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-slate-400 transition hover:bg-white hover:text-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-400"><X size={19} /></button></div><div className="space-y-7 overflow-y-auto p-4 sm:p-6">{avatarGroups.map((group) => <section key={group.category}><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-bold text-slate-800">{group.category}</h3><span className="text-xs font-semibold text-slate-400">{group.options.length} opções</span></div><div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8">{group.options.map((avatar) => { const index = avatarOptions.findIndex((option) => option.id === avatar.id); const selected = data.profile.avatar_url === avatar.src; return <button key={avatar.id} type="button" aria-label={`Selecionar avatar ${index + 1}`} aria-pressed={selected} disabled={uploading} onClick={() => void selectAvatar(avatar.id)} className={`group relative aspect-square overflow-hidden rounded-[18px] border-2 bg-white p-1.5 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-orange-400 hover:shadow-lg focus:outline-none focus-visible:ring-4 focus-visible:ring-orange-200 disabled:cursor-wait disabled:opacity-60 ${selected ? "border-orange-500 ring-4 ring-orange-100" : "border-white"}`}><Image src={avatar.src} alt="" width={256} height={256} className="h-full w-full rounded-[13px] object-cover transition duration-200 group-hover:scale-105" />{selected && <span className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-orange-500 text-white shadow-md"><Check size={14} strokeWidth={3} /></span>}</button>})}</div></section>)}</div><div className="flex items-center justify-between border-t border-orange-100 bg-white/80 px-5 py-4 sm:px-7"><p className="text-xs font-medium text-slate-500">128 avatares disponíveis</p><PremiumButton variant="secondary" className="min-h-10 rounded-[14px]" onClick={() => setAvatarOpen(false)}>Cancelar</PremiumButton></div></div></div>}
...
function Metric({ icon, value, label }: { icon: React.ReactNode; value: number | null; label: string }) { return <div className="flex min-h-[78px] items-center gap-3 rounded-[18px] border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-4 shadow-sm"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[14px] bg-orange-50 text-orange-600">{icon}</span><div><p className="text-2xl font-bold tracking-[-0.03em] text-slate-950">{value === null ? "—" : new Intl.NumberFormat("pt-BR").format(value)}</p><p className="mt-0.5 text-xs font-medium text-slate-500">{value === null ? "Indisponível agora" : label}</p></div></div>; }
function TextLink({ href, children }: { href: string; children: React.ReactNode }) { return <Link href={href} className="mt-5 inline-flex items-center gap-1 rounded-lg text-sm font-bold text-orange-600 transition hover:gap-2 hover:text-orange-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-400">{children}<ChevronRight size={16} /></Link>; }
